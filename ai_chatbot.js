/* ==========================================================================
   CARGOBAY ASSISTANT — GLOBAL LOGISTICS KNOWLEDGE BASE
   Covers: Countries, Ports, Airports, Shipping Routes, Risk Zones
   ========================================================================== */
...
const PORTS = {
  asia: [
    ...
    { name: 'Colombo', country: 'Sri Lanka', code: 'LKCMB', rank: 21, type: 'Transshipment Hub' },
...
/* ── Country & City Aliases for Universal Global Route Calculation ── */
const CITY_DIST_TO_ZRH = {
...
class CargoBayEngine {
  ...
  /* ── Universal Fare Calculator (Any Country/City to Any Country/City) ── */
  calculateFare(origin, destination, weightKg) {
    ...
    // Fallback info if city/country is not in dataset
    ...
    const wt = parseFloat(weightKg) || 100; // default 100kg

    // Distance calculation between any origin and destination
    let airKm = 5000;
    let seaKm = 8000;

    if (oData.km === 0) { // Origin is Zurich
      airKm = dData.km || 5000;
      seaKm = dData.sea_km || 8000;
    } else if (dData.km === 0) { // Destination is Zurich
      airKm = oData.km || 5000;
      seaKm = oData.sea_km || 8000;
    } else {
      // Non-Zurich to Non-Zurich route (e.g. Pakistan to UK, China to USA, Zurich to Dubai)
    ...
    // Air Freight: CHF 5–15/kg + distance factor
    ...
    // Road Freight (feasible within Europe / < 2500 km)
  ...
  respond(userText) {
    ...
    // ── 0. FARE / COST CALCULATOR (Universal Global Route Support) ──
    ...
    // ── 2. Simple Fast/Cheapest Route Advice ──
...
if (typeof window !== 'undefined') {
  window.cbEngine = new CargoBayEngine();
} else if (typeof global !== 'undefined') {
  global.cbEngine = new CargoBayEngine();
}

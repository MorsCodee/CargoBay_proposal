...
/* ------------------------------------------------------------------
   Fifth chart: Efficiency, Solvency & Coverage Ratios. These ratios
   span very different scales (0.2 to 9+), so instead of a multi-line
   chart we show one metric at a time via a dropdown. Percent-based
   ratios (Debt-to-Assets, Equity Ratio) are converted to decimals to
   stay consistent with the rest of the set. Interest Coverage is
   excluded per spec (it's a "x" multiple, not comparable here).
------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------
   ...
   Positioned using the exact pixel coordinates of the hovered dot
   (reported by that dot itself), so it always sits directly above
   the point -- never in a corner.
------------------------------------------------------------------- */
...
/* Custom dot: a small visible circle plus a larger invisible hit-area,
   so hovering is easy to trigger and reports its own pixel position. */
...
/* ------------------------------------------------------------------
   ...
   A native select's popup can open upward when it's short on space
   below, and its menu can't be given a border-radius reliably --
   so we build our own: a button + an absolutely positioned menu
   anchored below it, always opening downward, fully styled from the
   same CSS variables (including automatic dark-mode support).
------------------------------------------------------------------- */
...
export default function CargobayRevenue() {
  ...
  /* We position the floating tooltip ourselves (rather than relying on
     recharts' built-in <Tooltip>) because we only want to highlight the
     single segment under the cursor, not every stacked series at that
     x-position. Cell's onMouseEnter gives us the raw mouse event, which we
     convert into coordinates relative to the chart container. This only
     depends on documented Bar/Cell props, so it's stable across recharts
     versions (unlike reading recharts' internal axis-map objects). */
  ...
  return (
    <div className="cb-root">
      <style>{`
        :root {
        /* ---- Palette (freight-line inspired: sea, sky, tarmac, hazard-amber) ---- */
        --color-ink: #D8DCE6;          /* soft light ink - headings, text on dark surfaces */
        --color-air: #A8DADC;          /* muted steel blue - air freight */
        --color-sea: #037171;          /* muted teal - sea freight */
        --color-road: #C99A4A;         /* muted amber/gold - road freight */
        --color-rail: #414288;         /* muted violet - rail freight */
        --color-customs: #5CA084;      /* muted sage green - customs & docs */
        --color-warehousing: #D86326;  /* muted burnt orange - warehousing */
        --color-card-title: var(--color-warehousing);

        --color-bg: #0B0C10;           /* black page background */
        --color-surface: #000000;      /* black card / dropdown surface */
        --color-border: #2C303E;       /* subtle border on dark surfaces */
        --color-text: #D8DCE6;         /* primary text - soft light, not stark white */
        --color-text-muted: #868EA3;   /* muted text - dimmer gray */

        --color-highlight: #D8763F;         /* orange hover highlight for cards */
        --color-warehousing-light: #332720; /* dark tint of orange for row hover */
        ...
        /* ---- Global layout spec (identical to Summary1) ----
        ...
        /* ---- Spacing (same variable names/values as Summary1) ---- */
        ...
        .cb-root::before {
          ...
          opacity: 0.15; /* Adjust between 0.10 and 0.25 to control dimness */
          filter: brightness(0.7) contrast(1.1); /* Darkens the image extra */
          pointer-events: none;
          z-index: 0;
        }
        ...
        /* ---- Content wrapper: byte-for-byte the same box model as
           Summary1's .content-wrapper -- same properties, same order,
           same variable values, and the SAME 750px breakpoint where it
           swaps to the mobile padding values. ---- */
        ...
        /* ---- Container stripe: a small signature nod to shipping containers ---- */
        .cb-stripe {
          height: 6px;
        ...
        /* White tooltip box, tinted per hovered segment, floating on its own shadow */
        .cb-tooltip {
          position: absolute;
        ...
        /* Header row: orange, matching the graph's warehousing accent */
        .cb-table thead th {
          font-family: var(--font-heading);
          font-weight: 600;
          font-size: 16px;
        ...
        /* Footer row: transparent background, orange top border marking the total */
        .cb-table tfoot td {
          padding: 12px 16px;
          text-align: right;
          font-family: var(--font-heading);
        ...
        /* Container Grid Layout (3-column base) */
        .portfolio-grid {
          display: grid;
        ...
        /* Glassmorphic Card (Static - No Animations/Movement) */
